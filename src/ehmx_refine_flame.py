...
class RefineFlamePipeline(object):
    ...
    def optimize(self, track_frames, batch_base, id_share_params_result, optim_cfg,
        ...
        for i_step in t_bar:
            ...
            if i_step % 50 == 0:
                t_bar.set_description(
                    ...
                    f'Landmark Loss {landmark_loss2:.4f} | Reg Loss {reg_loss:.4f} | Temp Loss {temp_loss:.4f}'
                )
            
            if batch_imgs is not None and i_step % (steps - 1) == 0:
                self._save_visualization(batch_imgs, ret_dict, landmark_lst_dct, cameras, R, T, 
                                        head_lmk_valid, batch_id, i_step, 'flame')
        ...
        opt_p = torch.optim.Adam([{'params': [eye_pose_code], 'lr': 0.01}])
        ...
        for i_step in t_bar:
            ...
            if i_step % 50 == 0:
                t_bar.set_description(
                    f'Batch {batch_id} | Iter {i_step} => Loss {total_loss:.4f} | '
                    f'Landmark Loss {landmark_loss2:.4f} | Temp Loss {temp_loss:.4f}'
                )
            
            if batch_imgs is not None and i_step % (steps // 2) == 0:
                self._save_gaze_visualization(batch_imgs, t_gaze_landmarks, landmark_lst_dct, t_gaze_idx,
                                             cameras, R, T, head_lmk_valid, batch_id, i_step)
    ...
    def _save_visualization(self, batch_imgs, ret_dict, landmark_lst_dct, cameras, R, T,
                           head_lmk_valid, batch_id, i_step, prefix):
        """Save optimization visualization."""
        if self.saving_root is None:
            return
        ...
        k = 'lmk_mp'  # Use mediapipe landmarks for visualization
        for im_idx in img_indices:
            t_lmk = cameras.transform_points_screen(ret_dict[k], R=R, T=T)[..., :2]
            _img = batch_imgs[im_idx].clone().numpy().transpose(1, 2, 0).astype(np.uint8)
        ...
        cv2.imwrite(os.path.join(save_path, f"vis_fit_{prefix}_bid-{batch_id}_stp-{i_step}.png"),
                   cv2.cvtColor(vis_img.copy(), cv2.COLOR_RGB2BGR))
    
    def _save_gaze_visualization(self, batch_imgs, t_gaze_landmarks, landmark_lst_dct, t_gaze_idx,
                                cameras, R, T, head_lmk_valid, batch_id, i_step):
        """Save gaze optimization visualization."""
        if self.saving_root is None:
            return
        ...
        for im_idx in img_indices:
            t_lmk = cameras.transform_points_screen(t_gaze_landmarks[k], R=R, T=T)
            _img = batch_imgs[im_idx].clone().numpy().transpose(1, 2, 0).astype(np.uint8)
            _t_lmk_gz = t_lmk[im_idx, :, :2].detach().cpu().numpy()
    ...
    def run(self, tracked_rlt, id_share_params_result, optim_cfg, batch_imgs_dict=None, frame_interval=1):
        ...
        for batch_id, mini_batch in enumerate(mini_batchs):
            ...
            if batch_imgs_dict is not None:
                mini_batch_face_imgs = [batch_imgs_dict[key] for key in mini_batch if key in batch_imgs_dict]
